...
class TestRAGSystemIntegration:
    ...
    def test_query_without_tool_use(self, mock_rag_system):
        ...
        mock_rag_system.mock_ai_generator.generate_response.return_value = "Python is a programming language."

        response, sources = mock_rag_system.query("What is Python?", session_id="session_1")
    ...
    def test_query_with_tool_use(self, mock_rag_system, sample_search_results):
        ...
        mock_rag_system.mock_ai_generator.generate_response.return_value = "MCP is Model Context Protocol."

        # Mock sources from tool
        mock_rag_system.tool_manager.get_last_sources = MagicMock(return_value=[
        ...
        mock_rag_system.mock_ai_generator.generate_response.assert_called_once()
        call_kwargs = mock_rag_system.mock_ai_generator.generate_response.call_args[1]
        assert call_kwargs['tools'] == mock_rag_system.tool_manager.get_tool_definitions()
    ...
    def test_sources_tracking(self, mock_rag_system):
        ...
        mock_rag_system.mock_ai_generator.generate_response.return_value = "Answer based on course."
        ...
        mock_rag_system.tool_manager.get_last_sources = MagicMock(return_value=expected_sources)
    ...
    def test_conversation_history(self, mock_rag_system):
        ...
        mock_rag_system.mock_ai_generator.generate_response.return_value = "Follow-up answer"
        ...
        mock_rag_system.mock_session_manager.get_conversation_history.assert_called_once_with(session_id)
    ...
    def test_error_propagation(self, mock_rag_system):
        ...
        mock_rag_system.mock_ai_generator.generate_response.side_effect = Exception("API Error")
    ...
    def test_multiple_queries_same_session(self, mock_rag_system):
        ...
        # First query
        mock_rag_system.mock_ai_generator.generate_response.return_value = "First answer"
        response1, sources1 = mock_rag_system.query("First question", session_id=session_id)
        ...
        # Second query
        mock_rag_system.mock_ai_generator.generate_response.return_value = "Second answer"
        response2, sources2 = mock_rag_system.query("Second question", session_id=session_id)
        ...
        assert mock_rag_system.mock_session_manager.get_conversation_history.call_count == 2
...
class TestRAGSystemCourseManagement:
    ...
    def test_add_course_document(self, mock_rag_system_with_processor, sample_course_data, sample_course_chunks):
        ...
        course, chunk_count = mock_rag_system_with_processor.add_course_document("/path/to/course.txt")
        ...
        mock_rag_system_with_processor.vector_store.add_course_metadata.assert_called_once_with(sample_course_data)

        # Verify course content was added
        mock_rag_system_with_processor.vector_store.add_course_content.assert_called_once_with(sample_course_chunks)
